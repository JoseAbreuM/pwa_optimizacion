...
Las pruebas históricas se leen de `pruebas_pozo` por `id_pozo` y se conservan en la base offline global. En Muestras, la gráfica adicional de % AyS incluye solamente las pruebas cuya fuente es `OFM`; las pruebas OFM y las de otras fuentes tienen tablas independientes con búsqueda, orden y paginación. En Producción, petróleo es verde, agua azul y gas rojo. Cada gráfica permite fijar mínimos y máximos verticales; la gráfica de cruce representa cada tasa diaria como porcentaje de su máximo diario visible y muestra la tasa, el total mensual y la fuente en el tooltip. Las gráficas pueden exportarse como PNG desde móvil o escritorio. La ficha también genera un reporte local imprimible como PDF con las últimas dos bombas, producción, survey, muestras y, para pozos activos, parámetros recientes. El dashboard indica cuántos pozos tienen producción y pruebas OFM.

Las tres gráficas OFM de Producción y su cruce usan promedios diarios del período mensual: petróleo toma `bpd` de la vista, agua usa `agua / dias` y gas `gas / dias`; las consultas solo calculan tasas con `CASE WHEN dias > 0`. `petroleo`, `agua` y `gas` siguen siendo totales mensuales y se muestran junto con `dias`, completación, fecha y fuente en los tooltips. Los acumulados no intervienen. Los valores atípicos se conservan y se marcan con un anillo ámbar cuando la tasa supera Q3 + 1,5 × RIQ o los días activos caen por debajo de Q1 − 1,5 × RIQ entre los registros visibles de cada variable (con al menos cuatro registros). No se aplica un tope automático a la escala. Las unidades de los tres fluidos quedan sin etiqueta hasta verificarlas en la documentación oficial de la fuente. Las copias offline anteriores a esta versión necesitan una actualización manual para recibir `dias` y las tasas.
